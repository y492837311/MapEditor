        ...
        private void DrawMenuBar()
        {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                ...
                if (GUILayout.Button("View", EditorStyles.toolbarDropDown))
                {
                    ...
                    menu.AddItem(new GUIContent("Zoom Out"), false, () => zoomLevel = Mathf.Max(zoomLevel / 1.5f, 0.1f));
        ...
        private void DrawMainCanvas()
        {
            EditorGUILayout.BeginVertical(GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
            {
                ...
                EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
                {
                    ...
                    {
                        fillTolerance = EditorGUILayout.Slider("Tolerance", fillTolerance, 0f, 1f, GUILayout.Width(200));
                    }
                ...
                // 画布区域 - 使用 GetRect 而不是 GetLastRect
                Rect canvasArea = GUILayoutUtility.GetRect(0, 0, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
        ...
        private void DrawStatusBar()
        {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                ...
                if (IsMouseInCanvas(mousePos, canvasArea))
                {
                    ...
                    EditorGUILayout.LabelField($"Position: ({Mathf.FloorToInt(mapPos.x)}, {Mathf.FloorToInt(mapPos.y)})", GUILayout.Width(120));
                }
                else
                {
            ...
            else
            {
                ...
                switch (currentTool?.GetToolType())
                {
                    ...
                    case ToolType.Bucket:
                        EditorGUIUtility.AddCursorRect(new Rect(0, 0, position.width, position.height), MouseCursor.Arrow);
                        break;
...
// 添加全屏切换方法
        ...
        public int GetCurrentBlockId() => currentBlockId;
        ...
        public void ZoomToFit()
        {
            ...
            {
                // 基于高度计算缩放
                zoomLevel = (canvasArea.height * 0.95f) / (currentMapData.height * (canvasArea.height / currentMapData.height));
            }
            else
            {
                // 基于宽度计算缩放
                zoomLevel = (canvasArea.width * 0.95f) / (currentMapData.width * (canvasArea.width / currentMapData.width));
            }
